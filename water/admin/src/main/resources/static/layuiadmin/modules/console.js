  ...
  layui.use('table', function(){
    ...
    table.render({
      ...
      ,cols: [[
		  ...
		  ,{field: 'userNo', align: 'center', title: '户号'}
		  ,{field: 'status', align: 'center', title: '状态',templet: function(data){
			  if(data.status == 'IncidentStatus.New'){
				  return "新申请";
			  }else if(data.status == 'IncidentStatus.Processing'){
				  return "处理中";
			  }else if(data.status == 'IncidentStatus.Defer'){
				  return "延期";
			  }else{
				  return "";
			  }
		  }}
		  ,{field: 'deal', align: 'center', title: '处理状态',templet: function(data){
			  if(data.deal == true){
				  return "已处理";
			  }else{
				  return "未处理";
		  ...
		  ,{field: 'appdate',  title: '申请时间', templet:'<div>{{ layui.laytpl.toDateString(d.appdate) }}</div>'}
	    ]]
    	,skin: 'line'
    });
